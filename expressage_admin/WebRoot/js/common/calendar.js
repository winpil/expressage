...
L_calendar.prototype={
	...
	CreateHTML:function(){
		...
		for(var i=0;i<this.L_WDay.length;i++){
			htmlstr+="<li id=\"L_calendar-day_"+i+"\" style=\"border:1px solid #4DA2B6;\" onmouseover=\"this.style.background='#BCE6ED'\"  onmouseout=\"this.style.background='#FFFFFF'\"></li>\r\n";
			}
		htmlstr+="</ul>\r\n";
		htmlstr+="<span id=\"L_calendar-today\" onclick=\"parent."+this.NewName+".Today()\"><b><font color=\"navy\">今天<font></b></span>\r\n";
		htmlstr+="</div>\r\n";
		...
		stylestr+="#L_calendar{border:1px solid #4DA2B6;width:158px;padding:1px;height:180px;z-index:9998;text-align:center}\r\n";
		...
		stylestr+="#L_calendar-day li{cursor:pointer;width:18px !important;height:18px !important;width:20px;height:20px;float:left;;margin:1px;padding:0px;}\r\n";
		stylestr+="#L_calendar-control{height:25px;z-index:9998;}\r\n";
		stylestr+="#L_calendar-today{cursor:pointer;float:left;width:63px;height:20px;line-height:20px;margin:1px;text-align:center;text-decoration:underline;}"
		stylestr+="</style>";
  		...
  		for (i = 0; i < 39; i++)
		{
			...
			{ 
				if(i<firstday){
					...
					da.innerHTML="<b style=\"color:#198DB2\">" + this.L_WDay[i] + "</b>";
					month=(mm==1?1:mm);
					day=this.L_WDay[i];
					if(document.all){
	...
	SetDate:function(){
		...
		this.ClickObject.readOnly = true;
...
window.onload=function()
{
    ...
    frm.id = "L_DateLayer";
    frm.name="L_DateLayer";
    frm.frameBorder="0";
    frm.style.position="absolute";
    frm.style.width="160px";
    frm.style.height="188px";
